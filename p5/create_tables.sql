...
(
    ...
    address_id INT NOT NULL UNIQUE, -- ADDED: One station should map to one unique physical address/ID.
...
(
    ...
    license_plate NVARCHAR(20) UNIQUE NOT NULL, -- KEEP: License plate must be unique
    brand NVARCHAR(50),
    model NVARCHAR(50),
...
(
    ...
    plan_name NVARCHAR(50) NOT NULL UNIQUE, -- ADDED: Plan names should be unique
...
(
    ...
    card_number NVARCHAR(20) NOT NULL UNIQUE, -- ADDED: Card number must be unique system-wide
...
(
    ...
    wallet_account NVARCHAR(100) UNIQUE, -- ADDED: Wallet account identifier (e.g., email/ID) should be unique
    FOREIGN KEY (payment_method_id) REFERENCES Payment_Method(payment_method_id)
);
...
(
    ...
    charging_session_id INT NOT NULL UNIQUE, -- ADDED: One session should only have one invoice
    FOREIGN KEY (user_id) REFERENCES [User](user_id),
    FOREIGN KEY (user_subscription_id) REFERENCES User_Subscription(user_subscription_id),
...
(
    ...
    invoice_id INT NOT NULL UNIQUE, -- ADDED: One invoice should only have one payment record (or use a composite key if partial payments are allowed)
...
(
    ...
    skill_name NVARCHAR(100) UNIQUE NOT NULL, -- KEEP: Skill name is already unique
